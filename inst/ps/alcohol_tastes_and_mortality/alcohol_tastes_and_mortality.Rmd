...
This problem set mostly contains code chunks for programming tasks, but some tasks also contain quizzes. Whenever **Task** is displayed, a code chunk must be solved or is already given and needs to be executed. Sometimes code chunks must be solved entirely, and sometimes only certain parts of them, indicated by `___`.
...
In the first section, we will look at the descriptive statistics of the Russian Longitudinal Monitoring Survey (RLMS) participants.  We will also look at a summary for men and women to identify gender differences.
...
summary_males <- data_males_alc %>%
  ...
  group_by(Variable) %>%
  summarise(Observations = sum(!is.na(Value)),
...
Quiz: What impact do you think the ban had on alcohol sales?

(1) The sales of alcohol in liters have remained similar despite the ban
(2) The sales of alcohol in liters have risen
(3) The sales of alcohol in liters have fallen sharply

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Impact on sales")
```
<br>
...
Quiz: What other consequences could the ban have led to?

(1) The prices of alcoholic beverages have fallen
(2) The sales of illegal alcohol have increased
(3) Mortality has risen during the period of the anti-alcohol campaign

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Consequences")
```
<br>
...
Quiz: What would we expect if we assume that people who live in rural areas and turned 17 during the campaign have a higher vodka consumption than those who live in urban areas?

(1) The result of the DiD is negative
(2) The result of the DiD is zero
(3) The result of the DiD is positive

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("DiD results")
```
<br>
...
We see that the trends in vodka consumption rates are very similar for rural and urban areas in the years prior to the campaign. There is a small deviation in 1981 which may be explained by other developments that we cannot see here based on the averages.
In the years of the anti-alcohol campaign (gray box), we see that adolescents in rural areas have a more stable share of vodka, whereas in urban areas, this share collapses, indicating that the trends are different. 
...
Quiz: Which of the following statements is true for $\beta_1$?

(1) It is the intercept term
(2) It represents the differential effect of the campaign on rural adolescents who became 17 during the campaign, compared to urban ones who became 17
(3) It represents the differential effect of the campaign on rural adolescents who became 17 during the campaign, compared to rural ones who became 17 before or after the campaign
(4) It measures the general effect of becoming 17 years old during the campaign, regardless of rural or urban living status
...
However, it is questionable whether this model without controls and fixed effects is sufficient given possible weaknesses in the model assumptions. Therefore, we will now include further control variables and fixed effects in the regression model.
...
For a detailed description of cluster-robust standard errors that goes beyond this bachelor's thesis, see Cameron & Miller (2015).
...
- **reg4**: We add a further set of demographic control variables such as education, weight, subjective health status and marital status. We add `univ_educ`, `wtself`, `health_evaluation` and `married`. We also add our previously generated dummy `seventeen_before_1970`.
...
Quiz: Does the estimator become more or less precise with an increase in control variables?

(1) More precise
(2) Less precise
(3) The precision remains the same
...
Using this large import shock, we want to examine whether the long-term taste for alcohol can change to light alcohol if one was an adolescent at the time. This could also have an impact on mortality when consumers switch from hard to light alcohol. However, we need to approach the analyses with caution, as many other factors played a role during the time when the borders were opened, and not all of them can be taken into account.
...
Now we can run the regressions for this period. We regress on share of beer as the dependent variable and on share of vodka for a direct comparison. We use the full set of control variables from our previous regressions and cluster the standard errors at the individual level.
...
**Task:** The code is given. Just run the chunk below to run the regressions for the new period.
...
Quiz: If the results from the anti-alcohol campaign extend to this setting, what statement would be true?

(1) We should not observe a significant impact of the year an individual turned 17 on the share of beer consumed in samples that exclude the beer market expansion.
(2) The year in which an individual turned 17 should strongly influence the amount of beer consumed, even if the beer market didn’t expand.
(3) We should definitely observe a significant impact of the year an individual turned 17 on the share of beer consumed in samples that exclude the beer market expansion.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Placebo test")
```
<br>
...
Next, we run the regressions. To shrink the sample window, we use a loop that performs the regression for each 5-year time frame and then saves the coefficient and the standard errors in a data frame `results_beer_plac`. We also calculate the 95% confidence intervals.
...
results_beer_plac %>%
  ...
  labs(y = expression(phi ~ ": Coefficient for Year-Turned-17"), x = "Year in which individual turned 17") +
...
Quiz: Do the placebo test results look as expected if tastes form in early adulthood?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Placebo test results")
```
<br>
...
Quiz: Do you think there are big differences in mortality between Russia and the USA?
...
data5 <- data4 %>%
  group_by(year) %>%
  summarise(bu = mean(bu, na.rm = TRUE),
            samogon_share = mean(samogon_share, na.rm = TRUE),
            samogon_share_rur = mean(samogon_share_rur, na.rm = TRUE),
            .groups = 'drop') %>%
  ungroup()
  
data6 <- data5 %>%
  left_join(se_df, by = "year") %>%
  mutate(br = samogon_share_rur - bu,
         br_u = br + 1.96 * se_c,
         br_d = br - 1.96 * se_c)
...
For our analysis we need a loop that performs the described regression for each year from ages 10 - 35 and then saves the coefficient and the standard errors in a data frame `results_age`.
...
Next we run the regressions. To shrink the sample window, we use a loop that performs the regression for each time frame and then saves the coefficient and the standard errors in a data frame `results_beer`. For a comparison, we also run the regression for the share of vodka and save the results in `results_vodka`.
